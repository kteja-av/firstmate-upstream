...
| Busy state | No hook or plugin writer, so nothing is armed or seeded; `bin/fm-busy-lib.sh` folds the screen verdict as `humanlayer-anchor` and can prove tmux activity from live tool descendants of an identified foreground HumanLayer process (`humanlayer-process`). |
