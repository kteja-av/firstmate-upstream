...
| Herdr coverage | verified live on the herdr backend (0.8.0): a settled pane reads `idle humanlayer-anchor`, a running `sleep 90` tool call reads `busy humanlayer-process` through the foreground HumanLayer pids fed to the shared descendant walk, and pure model thinking (no child process) stays unknown; herdr's `pane process-info` foreground list does NOT include tool children, so the walk sources the TUI's pids from that list and the children from the system process table |
