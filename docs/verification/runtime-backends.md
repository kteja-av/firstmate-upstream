...
| Classification limit | captured `[Tool]` and `[Assistant]` rows can also be pending multiline input; the shared classifier therefore refuses a bare `>` continuation after pending text and never derives busy from these rows alone; tmux can report busy from live tool descendants of its identified foreground HumanLayer worker |
